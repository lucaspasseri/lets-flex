...
const stepRow = {
	...
	muscles: [{ id: 9, common_name: "Chest", scientific_name: "Pectoralis major", body_region: "torso", reference_url: "" }],
};

test("day mappers isolate database field names and preserve nested entities", () => {
	assert.deepEqual(toTrainingDay({
		id: 3, cycle_id: 2, program_id: 1, cycle_order: 1, day_order: 2,
		scheduled_date: "2026-08-20", label: "Push",
	}), {
		id: 3, cycleId: 2, programId: 1, cycleOrder: 1, dayOrder: 2,
		scheduledDate: "2026-08-20", label: "Push",
	});

	const session = toSessionMapperSeed({
		id: 4, name: "Upper", notes: "Controlled reps", is_archived: false, steps: [stepRow],
	});
...
test("workout mapper maps lifecycle fields and nullable step logs", () => {
	const workout = toWorkoutSession({
		id: 10, training_day_id: 3, session_id: 4, workout_session_order: 1,
		status: "planned", started_at: null, finished_at: null, notes: "",
		name: "Upper", is_archived: false, session_notes: "Template notes",
