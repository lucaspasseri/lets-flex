...
export default async function getDashboardPageData({ userId, programId, daysDifference, workoutSessionId, now = new Date() }) {
	const selectedDate = addDays(now, daysDifference ?? 0);
	const [userRow, programRow, trainingDayRow, cycleRows, workoutSessionRows] = await Promise.all([
		...
		programId ? trainingDaysRepository.findByProgramIdAndScheduledDate({ programId, scheduledDate: selectedDate }) : Promise.resolve(null),
		programId ? cyclesRepository.findAllByProgramId({ programId }) : Promise.resolve([]),
		programId ? workoutSessionsRepository.findAllByProgramId({ programId }) : Promise.resolve([]),
	]);

	const currentDayWorkoutSessionRows = trainingDayRow?.id
		? await workoutSessionsRepository.findAllByTrainingDayId({ trainingDayId: trainingDayRow.id })
		: [];
	...
	const mappedTrainingDay = trainingDayRow ? trainingDayMapper.toTrainingDay(trainingDayRow) : null;
	const currentTrainingDay = mappedTrainingDay && programId !== null ? {
		...mappedTrainingDay,
		programId,
		cycleOrder: cycles.find(/** @param {import("../cycles/cycles.types.js").Cycle} cycle */ cycle => cycle.id === mappedTrainingDay.cycleId)?.order ?? 0,
	} : null;
	const workoutSessions = workoutSessionRows.map(workoutSessionMapper.toWorkoutSession);
	const currentDayWorkoutSessions = currentDayWorkoutSessionRows.map(workoutSessionMapper.toWorkoutSession);
	const selection = resolveDashboardSelection({ workoutSessionId, trainingDay: currentTrainingDay, cycles, workoutSessions: currentDayWorkoutSessions });
	...
	return {
		...
		barChart: getBarChartData(currentProgram?.startDate ?? null, cycles, workoutSessions),
	};
}
