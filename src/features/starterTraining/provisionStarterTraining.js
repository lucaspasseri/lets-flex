...
/**
 * Ensures that one principal has the owner-specific starter hierarchy.
 ...
 */
export default async function provisionStarterTraining(
	{ userId, scheduledDate = format(new Date(), "yyyy-MM-dd") },
	db,
	dependencies = defaultDependencies,
) {
	const repositories = { ...defaultDependencies, ...dependencies };
	const owner = await repositories.usersRepository.findPrincipalByIdForUpdate(
		{ userId },
		db,
	);
	if (!owner) throw new Error("Starter training owner is unavailable");

	const existing = await repositories.programsRepository.findStarterWorkspace(
		{ userId, provisioningKey: starterWorkoutManifest.provisioningKey },
		db,
	);
	if (existing) {
		return {
			...
			trainingDayId: existing.training_day_id,
			workoutSessionId: existing.workout_session_id,
		};
	}
