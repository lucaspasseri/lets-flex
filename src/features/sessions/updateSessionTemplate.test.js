...
function createDependencies({
	sessionExists = true,
	existingStepExists = true,
	failCreate = false,
} = {}) {
	/** @type {any[]} */
	const calls = [];
	const client = {
		async query(/** @type {any} */ sql) {
			calls.push(sql);
		},
		release() {
			calls.push("RELEASE");
	...
	return {
		...
		dependencies: {
			...
			sessionStepsRepository: {
				async moveOrdersOutOfWay(/** @type {any} */ received, /** @type {any} */ db) {
					calls.push(["move-orders", received, db]);
				},
				async update(/** @type {any} */ received, /** @type {any} */ db) {
					calls.push(["update-step", received, db]);
					return existingStepExists;
				},
				async create(/** @type {any} */ received, /** @type {any} */ db) {
					calls.push(["create-step", received, db]);
					if (failCreate) throw new Error("step failed");
					return { id: 20 };
				},
				async deleteExcept(/** @type {any} */ received, /** @type {any} */ db) {
					calls.push(["delete-except", received, db]);
				},
			},
		},
	};
...
test("session update preserves retained step identities and synchronizes order, additions, and removals", async () => {
	...
	const retained = calls.find(
		(call) => Array.isArray(call) && call[0] === "update-step",
	)[1];
	const added = calls.find(
		(call) => Array.isArray(call) && call[0] === "create-step",
	)[1];
	const cleanup = calls.find(
		(call) => Array.isArray(call) && call[0] === "delete-except",
	)[1];
...
test("missing sessions and stale step identities return not found and roll back", async () => {
	for (const options of [{ sessionExists: false }, { existingStepExists: false }]) {
		...
		await assert.rejects(
			updateSessionTemplate(input, /** @type {any} */ (dependencies)),
			SessionTemplateNotFoundError,
		);
		assert.deepEqual(
			calls.filter((call) => typeof call === "string"),
