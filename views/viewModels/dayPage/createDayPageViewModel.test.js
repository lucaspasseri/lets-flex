...
test("day page creates predictable navigation, form, cards, and modal contracts", () => {
	...
	assert.equal(result.components.workoutSessionList.items[0].header.media?.src, null);
	assert.equal(
		result.components.workoutSessionList.items[0].steps[0].title,
		"Barbell:",
	);
	assert.equal(result.components.workoutSessionList.items[0].steps[0].media.src, null);
	assert.equal(
		result.components.workoutSessionList.items[0].header.media?.initial,
		"A",
	);
	assert.equal(
		result.components.workoutSessionList.items[0].steps[0].media.initial,
		"B",
	);
