...
const page = { path: "/day", url: "/programs/day?dayId=2", backUrl: "/programs", backUrlWithoutParams: "/programs", title: "Day" };
const user = { id: 1, name: "Lucas", dateOfBirth: null, anamnesis: null };
const days = [1, 2, 3].map(id => ({
	id, cycleId: 5, programId: 6, cycleOrder: 1, dayOrder: id,
	scheduledDate: `2026-08-${String(19 + id).padStart(2, "0")}`, label: `Day ${id}`,
}));
const step = {
	id: 8, name: "Bench", order: 1, type: "Strength", sets: 3, reps: 8,
...
test("day page creates predictable navigation, form, cards, and modal contracts", () => {
	const result = createDayPageViewModel({
		...
		data: {
			...
			workoutSessions: { items: [
				{ id: 30, trainingDayId: 2, sessionId: 20, order: 1, status: "planned", startedAt: null, finishedAt: null, notes: null, name: "Available", sessionNotes: "Notes", isArchived: false, steps: [{ ...step, stepLog: null }] },
				{ id: 31, trainingDayId: 2, sessionId: 20, order: 2, status: "cancelled", startedAt: null, finishedAt: null, notes: null, name: "Cancelled", sessionNotes: null, isArchived: false, steps: [] },
			] },
		},
	});
	...
	assert.deepEqual(result.components.sessionLinkForm.fields.session.options, [{ label: "Available", value: 20 }]);
	...
	assert.equal(result.components.workoutSessionList.items[0].steps[0].title, "Barbell:");
	assert.equal(result.components.workoutSessionList.items[0].steps[0].prescriptionLabel, "3 sets × 8 reps");
	assert.equal(result.components.workoutSessionList.cancelModals[0].form.trainingDayId, 2);
});
...
test("day page exposes safe empty states for an invalid selection", () => {
	const result = createDayPageViewModel({
		...
		data: { currentUser: null, days: { current: null, items: [] }, sessions: { items: [] }, workoutSessions: { items: [] } },
	});
...
test("day template renders only from its component ViewModels", async () => {
	const viewModel = createDayPageViewModel({
		...
		data: {
			currentUser: user,
			days: { current: days[1], items: days },
			sessions: { items: [{ id: 20, name: "Available", notes: "", isArchived: false, steps: [step] }] },
			workoutSessions: { items: [{
				id: 30, trainingDayId: 2, sessionId: 20, order: 1, status: "planned",
				startedAt: null, finishedAt: null, notes: null, name: "Available",
				sessionNotes: "Notes", isArchived: false, steps: [{ ...step, stepLog: null }],
			}] },
		},
	});
	...
	const [html, emptyHtml] = await Promise.all([viewModel, emptyViewModel].map(model =>
	...
	assert.match(html, /class="day-card active">21\/08\/2026/);
	assert.match(html, /<option value="20">\s*Available/);
	assert.match(html, /session-card-workout/);
	...
	assert.doesNotMatch(emptyHtml, /session-card-workout/);
});
