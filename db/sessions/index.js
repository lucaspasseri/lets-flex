...
async function getSessionWithExerciseInfoById(db, { sessionId }) {
	const { rows } = await db.query(
		`
		SELECT 
			se.id,
...
async function archiveSessionTemplateById(db, { sessionId }) {
	await db.query(
		`
			UPDATE sessions
...
export {
	insertSession,
	getAllSessions,
	getAllSessionsWithExerciseInfo,
	getSessionWithExerciseInfoById,
	getSessionById,
	archiveSessionTemplateById,
};
